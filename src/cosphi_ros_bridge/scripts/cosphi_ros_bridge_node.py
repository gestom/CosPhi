...
def main():
   ...
   while not rospy.is_shutdown():
      ...
      #parse to extract the coordinates of each robot , add them to the standerd form then publish them on a topic defined by the robot ID
      for index in range(len(Array_Robots)-1):
         Array_All = Array_Robots[index].split()
         if Array_All[0] == "Detected":
             frameTime = Array_All[5]
         elif Array_All[0] == "Robot":
            ID = Array_All[1]
            #publish only fresh positions
            if Array_All[5] == frameTime:
                if float(ID) < 0:
                    ID = 'Unknown'
